...
function Linux.Create(config)
    ...
    function LinuxLib.Tab(config)
        ...
        -- CORREÇÃO: Função de seleção de aba melhorada
        ...
        -- CORREÇÃO: Verifica se esta aba deve ser a padrão e a ativa imediatamente
        if config.Default == true then
            DefaultTab = tabIndex
            -- Ativa a aba imediatamente se for marcada como padrão
            task.spawn(function()
                task.wait(0.1) -- Pequeno delay para garantir que tudo foi criado
                SelectTab()
            end)
        elseif CurrentTab == nil and tabOrder == 1 then
            -- Se não há aba padrão definida, ativa a primeira aba criada
            DefaultTab = tabIndex
            task.spawn(function()
                task.wait(0.1)
                SelectTab()
